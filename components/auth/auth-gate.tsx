...
export function AuthGate({ children }: Props) {
  ...
  return <main className="auth-shell"><div className="auth-grid" aria-hidden="true" /><section className="auth-card"><div className="auth-logo-wrap"><img className="auth-logo" src="/icon.svg" alt="JF Radar" /></div><div className="auth-brand">JF <span>RADAR</span></div><p className="auth-kicker">MONITORAMENTO METEOROLÓGICO</p><h1>{title}</h1><p className="auth-muted">Previsão e radar em tempo real para Juiz de Fora.</p>
    ...
    <button type="button" className="auth-google" onClick={googleLogin} disabled={busy}><span className="google-mark" aria-hidden="true">G</span><span>Continuar com Google</span></button>{mode === "login" && <a className="auth-access" href="https://wa.me/5532987062517?text=Oi%2C%20desejo%20ter%20acesso%20ao%20sistema%3A%20jf-radar.vercel.app" target="_blank" rel="noreferrer"><MessageCircle size={18} aria-hidden="true" /><span>Solicitar acesso</span></a>}{error && <p className="auth-error" role="alert">{error}</p>}
