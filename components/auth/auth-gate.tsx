...
export function AuthGate({ children }: Props) {
  ...
  useEffect(
    () =>
      onAuthStateChanged(clientAuth, (next) => {
        setUser(next);
        setChecking(false);
      }),
    [],
  ...
  useEffect(() => {
    let active = true;
    const value =
      new URLSearchParams(window.location.search).get("code")?.trim() || null;
    setCode(value);
    ...
    fetch(`/api/auth/invite?code=${encodeURIComponent(value)}`, {
      cache: "no-store",
    })
      .then(async (response) => {
  ...
  const title = useMemo(
    () => (mode === "login" ? "Acesse o JF Radar" : "Crie seu acesso"),
    [mode],
  );
  ...
  if (checking || (mode === "register" && inviteState === "checking"))
    return (
      <div className="auth-loading" role="status" aria-live="polite">
        <div className="auth-loading-mark">
          <span className="auth-loading-ring auth-loading-ring-back" />
          <span className="auth-loading-ring auth-loading-ring-front" />
          <img src="/icon.png" alt="JF Radar" />
          <i />
        </div>
        <p>{mode === "register" ? "VALIDANDO CONVITE" : "CARREGANDO ACESSO"}</p>
      </div>
    );
  ...
  async function register(event: React.FormEvent) {
    ...
    if (inviteState !== "valid")
      return setError("É necessário um convite válido para criar uma conta.");
    ...
    try {
      ...
      const response = await fetch("/api/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ idToken, code, name: form.name, newUser: true }),
      });
      if (!response.ok) {
      ...
      setError(
        cause instanceof Error && cause.message === "INVITE_INVALID"
    ...
    <main className="auth-shell">
      ...
      <section className="auth-card">
        <div className="auth-logo-wrap">
          <img className="auth-logo" src="/icon.png" alt="JF Radar" />
        </div>
        <div className="auth-brand">
          JF <span>RADAR</span>
        </div>
        <p className="auth-kicker">MONITORAMENTO METEOROLÓGICO</p>
        <h1>{title}</h1>
        <p className="auth-muted">
          Previsão e radar em tempo real para Juiz de Fora.
        </p>
        {mode === "login" ? (
          ...
          <form onSubmit={register} className="auth-form">
            ...
            <button className="auth-primary" disabled={busy || invalidInvite}>
        ...
        {mode === "login" && (
          <a
            ...
            href="https://wa.me/5532987062517?text=Oi%2C%20desejo%20ter%20acesso%20ao%20sistema%3A%20jf-radar.vercel.app"
            target="_blank"
            rel="noreferrer"
          >
        ...
        {mode === "register" && (
          <button className="auth-link" onClick={closeInviteMessage}>
            Voltar para login
          </button>
        )}
        {mode === "login" && code && inviteState === "invalid" && (
          <div className="auth-dialog">
            <strong>Código expirado ou inválido</strong>
            <span>Este link de convite não permite criar uma conta.</span>
            <button onClick={closeInviteMessage}>Fechar</button>
          </div>
        )}
      </section>
    </main>
