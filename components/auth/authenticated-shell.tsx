...
export function AuthenticatedShell({ user, children }: { user: User; children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (Notification.permission === "default") void user.getIdToken().then((idToken) => registerPushToken(user.uid, idToken)).then((result) => {
  ...
  async function toggleNotifications() {
    ...
    const result = await registerPushToken(user.uid, idToken);
    if (!result.ok) {
      const messages = {
